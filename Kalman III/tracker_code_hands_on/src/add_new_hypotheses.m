function [assignments, objects, nobjects] = add_new_hypothese(assignments, objects, objects_map, pos_observed, image, NEW_HYP_DUMMY_COST)
%[assignments, objects, nobjects] = add_new_hypothese(assignments, objects, objects_map, pos_observed, image, NEW_HYP_DUMMY_COST)
...
if(nrows < ncols)
  lap_new_hyp_cost = [new_hyp_cost; ones(ncols - nrows, ncols) * NEW_HYP_DUMMY_COST];
end

if(ncols < nrows)
  lap_new_hyp_cost = [new_hyp_cost, ones(nrows, nrows - ncols) * NEW_HYP_DUMMY_COST];
end

% Add further dummies to avoid matching unlikely pairs. 
lap_new_hyp_cost = [lap_new_hyp_cost, ones(size(lap_new_hyp_cost)) * NEW_HYP_DUMMY_COST;
	          ones(size(lap_new_hyp_cost)) * NEW_HYP_DUMMY_COST, ones(size(lap_new_hyp_cost)) * NEW_HYP_DUMMY_COST];

% Run the LAP assignment algorithm to minimise the cost
